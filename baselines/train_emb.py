...
parser.add_argument('--raw-sam', action='store_true', default=False)
...
# raw_sam_mode = 'conv'
if args.raw_sam:
    embname = f'samvit_{raw_sam_mode}'
...
class EmbDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if args.raw_sam:
            samfile = f'/u/project/jflint/adityago/bmri_samembed/{base_name}.npz'
            if not os.path.exists(samfile):
                evec = np.zeros(552*256*16*16)
            else:
                evec = np.load(samfile)['arr_0']
                evec = evec.reshape(-1)
            if raw_sam_mode == 'conv':
                evec = evec.reshape(552, 256, 16, 16)
        else:
...
if False:
    class ConvMLP(nn.Module):
        def __init__(self, outdim, hdim=64):
            super().__init__()

            self.full_fc = nn.Sequential(
                nn.Linear(552*256*16*16, hdim),
                nn.ReLU(),
                nn.Linear(hdim, hdim),
                nn.ReLU(),
    ...
    class ConvMLP(nn.Module):
        def __init__(self, outdim, hdim=64):
            ...
            self.conv = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Conv2d(8, 2, kernel_size=3, padding=1, stride=2),
            )

            self.full_fc = nn.Sequential(
                nn.Linear(552*2*4*4, hdim),
                nn.ReLU(),
                nn.Linear(hdim, hdim),
                nn.ReLU(),
                nn.Linear(hdim, outdim),
        ...
        def forward(self, x):
            bsize = len(x)
            x = self.conv(x.view(-1, 256, 16, 16))
            x = x.view(bsize, 552, -1)
            x = x.view(len(x), -1)
            x = self.full_fc(x)
            return x
...
loaders = { ph: torch.utils.data.DataLoader(
                ...
                shuffle=ph == 'train', num_workers=0 if args.raw_sam else 2) for ph, ds in dsets.items() }
